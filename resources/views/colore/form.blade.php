<div class="container mx-auto">
    <div class="flex flex-col items-center p-4">
        <div class="w-full">
            <div class="mb-6">
                <label for="codigo" class="block text-lg font-medium text-white">{{ __('Codigo') }}</label>
                <input type="text" name="codigo" id="codigo" placeholder="Codigo"
                    value="{{ old('codigo', $colore?->codigo) }}"
                    class="mt-2 px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-lg border-gray-300 rounded-md transition duration-150 ease-in-out @error('codigo') border-red-500 @enderror ">
                {!! $errors->first('codigo', '<p class="text-red-500 text-lg mt-1">:message</p>') !!}
            </div>
            <div class="mb-6">
                <label for="nombre" class="block text-lg font-medium text-white">{{ __('Nombre') }}</label>
                <input type="text" name="nombre" id="nombre" placeholder="Nombre"
                    value="{{ old('nombre', $colore?->nombre) }}"
